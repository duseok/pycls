...
class MBConv(Module):
  ...
  def __init__(self, w_in, exp_r, stride, w_out):
    ...
    self.use_res_connect = self.stride==1 and w_in==w_out # check skip connection
...
class MNV2Stage(Module):
  """MobileNetV2 stage."""

  def __init__(self, w_in, exp_r, stride, w_out, d): 
    super(MNV2Stage, self).__init__()
...
class MobileNetV2(Module):
  ...
  def __init__(self, params=None):
    ...
    p = MobileNetV2.get_params() if not params else params
    ...
    for i, (d, w, exp_r, stride) in enumerate(stage_params):
      stage = MNV2Stage(prev_w, exp_r, stride, w, d)
      self.add_module("s{}".format(i + 1), stage)
      prev_w = w
    self.head = MNV2Head(prev_w, hw, nc)
  ...
  def complexity(cx, params=None):
    ...
    for d, w, exp_r, stride in stage_params:
      cx = MNV2Stage.complexity(cx, prev_w, exp_r, stride, w, d)
      prev_w = w
    cx = MNV2Head.complexity(cx, prev_w, hw, nc)
    return cx
